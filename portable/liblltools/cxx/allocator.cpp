...
namespace LLTools {
    ...
    uint8_t* FirstFitAllocator::allocate(size_t size) {
    ...
    MemoryBlockHeader* FirstFitAllocator::split(MemoryBlockHeader* mbh, size_t size) {
    ...
    void FirstFitAllocator::merge(MemoryBlockHeader* mbh1, MemoryBlockHeader* mbh2) {
        if(!mbh1 || !mbh2) return;
    ...
    uintptr_t FirstFitAllocator::alignAddress(uintptr_t addr) {
        return alignUp(addr, heapAlignment);
    }

    uintptr_t FirstFitAllocator::dataStart(MemoryBlockHeader* mbh) {
        if(!mbh) return 0;

        return (mbh->magic == memoryBlockMagic) ? reinterpret_cast<uintptr_t>(mbh) + sizeof(MemoryBlockHeader) : 0;
    }

    uintptr_t FirstFitAllocator::dataEnd(MemoryBlockHeader* mbh) {
        if(!mbh) return 0;

        return (mbh->magic == memoryBlockMagic) ? dataStart(mbh) + mbh->size : 0;
    }

    uintptr_t FirstFitAllocator::initMemory(uintptr_t heapStartAddr, uintptr_t heapEndAddr) {
    ...
    uint8_t* FirstFitAllocator::getData(MemoryBlockHeader* mbh) {
        return reinterpret_cast<uint8_t*>(dataStart(mbh));
    }

    MemoryBlockHeader* FirstFitAllocator::getHeader(uint8_t* ptr) {
